...
public class Progress {
  ...
  public void complete() {
    ...
    if (myParent != null) {
      ...
      // We don't need transactional semantics, so we're STOPPED doing this. 
  ...
  /** Adds a node with a specified progress weightage to the tree. */
  public synchronized void addPhases(float[] weightages) {
    // FIXME reconsider others?
    for ( int i = 0; i < weightages.length; ++i ) {
        addNewPhase();
        progressWeightagesForPhases.add(weightages[i]);
    }
    fixedWeightageForAllPhases = false;
  }
}
