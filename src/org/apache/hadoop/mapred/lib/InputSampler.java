...
public class InputSampler<K,V> extends Configured implements Tool  {
  ...
  public static class SplitSampler<K,V> implements Sampler<K,V> {
    ...
    public K[] getSample(InputFormat<K,V> inf, JobConf job) 
        throws IOException, InterruptedException {
      List<InputSplit> splits = Arrays.asList(inf.getSplits(job,0));
      ...
      for (int i = 0; i < splitsToSample; ++i) {
          InputSplit split = splits.get(i * splitStep);
          RecordReader<K,V> reader = inf.getRecordReader(split, job, new DummyReporter());
  ...
  public static <K,V> void writePartitionFile(JobConf job, Sampler<K,V> sampler) 
    ...
    SequenceFile.Writer writer = SequenceFile.createWriter(fs, 
      job, dst, job.getMapOutputKeyClass(), NullWritable.class);
