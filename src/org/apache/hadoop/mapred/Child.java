...
import skewtune.mapreduce.protocol.SilentSkewTuneTaskUmbilicalProtocol;
...
class Child {
  ...
  public static void main(String[] args) throws Throwable {
    ...
    // INITIALIZE SKEWREDUCE UMBILICAL
    SkewTuneTaskUmbilicalProtocol tmpumbilical = null;
    final InetSocketAddress srttaddr = NetUtils.createSocketAddr(defaultConf.get(TTConfig.TT_REPORT_ADDRESS, "127.0.0.1:60100"));
    try {
        tmpumbilical = taskOwner.doAs(new PrivilegedExceptionAction<SkewTuneTaskUmbilicalProtocol>() {
            @Override
            public SkewTuneTaskUmbilicalProtocol run() throws Exception {
                return (SkewTuneTaskUmbilicalProtocol) RPC.getProxy(
                        SkewTuneTaskUmbilicalProtocol.class,
                        SkewTuneTaskUmbilicalProtocol.versionID, srttaddr,
                        defaultConf);
            }
        });
    } catch (IOException e) {
        LOG.error("Unable to initiate connection to skewreduce task tracker",e);
        tmpumbilical = null;
    }
    final SkewTuneTaskUmbilicalProtocol srumbilical = new SilentSkewTuneTaskUmbilicalProtocol(tmpumbilical);
    ...
    try {
        ...
        childUGI.doAs(new PrivilegedExceptionAction<Object>() {
          ...
          public Object run() throws Exception {
            try {
              ...
              taskFinal.run(job, umbilical, srumbilical);             // run the task
            } finally {
              TaskLog.syncLogs(logLocation, taskid, isCleanup);
            }
      ...
      if ( srumbilical != null ) RPC.stopProxy(srumbilical);
