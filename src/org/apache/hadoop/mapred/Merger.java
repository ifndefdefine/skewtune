...
public class Merger {  
  ...
  RawKeyValueIterator merge(Configuration conf, FileSystem fs, 
                            ...
                            Counters.Counter writesCounter,
                            Progress mergePhase,
                            MergerProgressListener listener)
      throws IOException {
    return merge(conf, fs, keyClass, valueClass, segments, mergeFactor, tmpDir,
                 comparator, reporter, false, readsCounter, writesCounter,
                 mergePhase, listener);
  ...
  RawKeyValueIterator merge(Configuration conf, FileSystem fs,
                            ...
                            Progress mergePhase)
      throws IOException {
    return merge(conf,fs,keyClass,valueClass,segments,mergeFactor,tmpDir,comparator,reporter, sortSegments, readsCounter, writesCounter, mergePhase,null);
  }
  ...
  RawKeyValueIterator merge(Configuration conf, FileSystem fs,
    ...
    return new MergeQueue<K, V>(conf, fs, segments, comparator, reporter,
                           sortSegments, listener).merge(keyClass, valueClass,
                                               mergeFactor, tmpDir,
                                               readsCounter, writesCounter,
                                               mergePhase);
  }
  ...
  RawKeyValueIterator merge(Configuration conf, FileSystem fs,
      ...
      throws IOException {
    return new MergeQueue<K, V>(conf, fs, segments, comparator, reporter,
                           sortSegments, codec, listener).merge(keyClass, valueClass,
                                               mergeFactor, tmpDir,
                                               readsCounter, writesCounter,
                                               mergePhase);
  }
  ...
  extends PriorityQueue<Segment<K, V>> implements RawKeyValueIterator {
    ...
    private MergerProgressListener listener;
    ...
    // SKEWREDUCE
    final long interruptCutoff;
    long finalMergeSize; // only used for reduce
    ...
    throws IOException {
      ...
      long bw = conf.getLong(SkewTuneJobConfig.SKEWTUNE_REPARTITION_DISK_BW, 30 << 20);
      float t = conf.getFloat(SkewTuneJobConfig.SKEWTUNE_REPARTITION_MAX_SCAN_TIME, 60.f);
      this.interruptCutoff = (long) Math.ceil(bw*t);
    }
    ...
    public MergeQueue(Configuration conf, FileSystem fs, 
            List<Segment<K, V>> segments, RawComparator<K> comparator,
            Progressable reporter, boolean sortSegments, MergerProgressListener listener) {
    ...
    private void adjustPriorityQueue(Segment<K, V> reader) throws IOException{
      ...
      if ( listener != null ) {
          listener.setProcessedBytesByKey(totalBytesProcessed);
      }
    ...
    public boolean next() throws IOException {
      ...
      if ( listener != null ) {
          listener.setProcessedBytesByValue(totalBytesProcessed);
      }
    ...
    RawKeyValueIterator merge(Class<K> keyClass, Class<V> valueClass,
      ...
      if ( Thread.interrupted() ) {
          if ( (totalBytes+finalMergeSize) > interruptCutoff ) {
              InterruptedIOException iioex = new InterruptedIOException("SORT interrupted");
              iioex.bytesTransferred = (int)totalBytesProcessed;
              throw iioex;
          }
          // if we are going to proceed, 
      }
      ...
      if ( listener != null ) {
          listener.update(totalBytesProcessed, totalBytes);
      }
      ...
      do {
          ...
          for (Segment<K, V> segment : mStream) {
            ...
            if (hasNext) {
              ...
              numSegments--; //we ignore this segment for the merge
                  ...
                  LOG.error("problem in "+segment.file+" pos="+segment.segmentOffset+" len="+segment.segmentLength+" ",ioe);
                  throw ioe;
              }
          }
        ...
        if (numSegments <= factor) {
            long finalMergeBytes = 0;
            for (int i = 0; i < segmentsToMerge.size(); i++) {
              finalMergeBytes += segmentsToMerge.get(i).getLength();
            }
          ...
          if ( listener != null ) {
              listener.beginFinalMerge(finalMergeBytes, totalBytesProcessed);
          }
        ...
        if ( Thread.interrupted() ) {
            // FIXME interrupt or not? remain sort + final merge bytes is larger than X bytes
            if ( (totalBytes - totalBytesProcessed)+finalMergeSize > interruptCutoff ) {
                InterruptedIOException iioex = new InterruptedIOException("SORT interrupted");
                iioex.bytesTransferred = (int)totalBytesProcessed;
                throw iioex;
            }
            // otherwise, we will continue and complete the merge
        }
      } while(true);
    }
    ...
    long computeBytesInMerges(int factor, int inMem) {
      ...
      if ( ! includeFinalMerge ) {
          for ( long sz : segmentSizes ) {
              this.finalMergeSize += sz;
          }
      }
