...
public class JobInProgress {
  ...
  public JobInProgress(JobTracker jobtracker, 
    ...
    // SKEWREDUCE
    this.handOverJobs = new HashSet<JobID>();
    String parentJobID = conf.get("mapreduce.job.parent");
    if ( parentJobID != null ) {
        JobID pjobid = JobID.forName(parentJobID);
        this.parentJip = jobtracker.getJob(pjobid);
        this.parentJip.addHandoverJob(this.jobId, conf.getBoolean("mapreduce.job.parent.sticky",false));
    }
  }
  ...
  private synchronized boolean canLaunchJobCleanupTask() {
    ...
    if ( !handOverJobs.isEmpty() || sticky ) {
        return false;
    }
...
//        if ( this.speculationNotifyUri == null ) {
            tip = getSpeculativeMap(taskTrackerName, taskTrackerHost);
            if ( tip != null ) {
                return tip.getIdWithinJob();
            }
//        } else {
//          if ( this.speculatedTasks.get(tip.getIdWithinJob()) ){
              // just forward this event
//          SpeculationEventNotifier.registerNotification(conf, tip);
//              speculatedTasks.set(tip.getIdWithinJob());
//          }
//      }
    }
   return -1;
...
//        } else {
//            // we are forwarding this
//          if ( speculatedTasks.get(tip.getIdWithinJob()+numMapTasks) ) {
//              // just forward this event
//              SpeculationEventNotifier.registerNotification(conf, tip);
//              speculatedTasks.set(tip.getIdWithinJob()+numMapTasks);
//          }
//      }
    }
  ...
  private void jobComplete() {
    ...
    if (this.status.getRunState() == JobStatus.RUNNING ||
      ...
      // FIXME if this is a handover job of some other job, 
      // remove this from the original task so that it can continue
      if ( parentJip != null ) {
          parentJip.handOverJobComplete(this.jobId,JobStatus.SUCCEEDED);
      }
  ...
  private synchronized void terminateJob(int jobTerminationState) {
    if ((status.getRunState() == JobStatus.RUNNING) ||
      ...
      // FIXME if this is a handover task of some other job, 
      // mark it as unsuccessful. if it were killed or failed,
      // adjust the original task accordingly -- set jobKilled or jobFailed
      if ( parentJip != null ) {
          parentJip.handOverJobComplete(this.jobId,jobTerminationState);
      }
  ...
  // SKEWREDUCE
  String speculationNotifyUri;
  BitSet speculatedTasks;
  JobInProgress parentJip;
  Set<JobID> handOverJobs; 
  boolean sticky; // if this flag is turned on, it will stay as long as all subtasks are successful
  static final ExecutorService execCompletion;

  public synchronized void handOverJobComplete(JobID jobid,int state) {
      this.handOverJobs.remove(jobid);
...
//      }
      if ( state == JobStatus.FAILED ) {
          sticky = false; // make it complete
          execCompletion.submit(new Runnable() {
            @Override
            public void run() {
                fail();
            }
  ...
  public synchronized void addHandoverJob(JobID jobid,boolean sticky) {
      this.handOverJobs.add(jobid);
      this.sticky = sticky;
  }
  
  static {
//      SpeculationEventNotifier.startNotifier();
      execCompletion = Executors.newSingleThreadExecutor(new ThreadFactory() {
