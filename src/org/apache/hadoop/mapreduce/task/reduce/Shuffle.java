...
import org.apache.hadoop.mapred.TaskProgress;
...
public class Shuffle<K, V> implements ExceptionReporter {
  ...
  public Shuffle(TaskAttemptID reduceId, JobConf jobConf, FileSystem localFS,
                 ...
                 Progress mergePhase,
                 Task reduceTask,
                 SkewTuneTaskUmbilicalProtocol srumbilical,
  ...
  public RawKeyValueIterator run() throws IOException, InterruptedException {
    ...
    final SkewReduceEventFetcher<K,V> srEventFetcher = reactiveShuffle ? new SkewReduceEventFetcher<K,V>(reduceId, srumbilical, scheduler, this) : null;
    if ( reactiveShuffle )
        srEventFetcher.start();
    ...
    if ( reactiveShuffle ) {
        srEventFetcher.interrupt();
        try {
          srEventFetcher.join();
        } catch(Throwable t) {
          LOG.info("Failed to stop " + srEventFetcher.getName(), t);
        }    
    }
