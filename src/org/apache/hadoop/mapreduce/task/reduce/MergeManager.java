...
import org.apache.hadoop.mapred.MergerProgressListener;
...
public class MergeManager<K, V> {
  ...
  /**
   * Out of Memory watermark. initially set to max, then gradually reduced
   * per Out of Memory exception in reserve method.
   */
  private long oomWaterMark = Long.MAX_VALUE;
  ...
  private final MergerProgressListener listener;
  ...
  public boolean canShuffleToMemory(long requestedSize) {
    return (requestedSize < maxSingleShuffleLimit); 
  }
  
  final private MapOutput<K,V> stallShuffle = new MapOutput<K,V>(null);
  
  public enum ReservePreference {
      MEMORY,
      PREFER_MEMORY,
      DISK
  }
  ...
  public MapOutput<K,V> reserve(TaskAttemptID mapId, 
      ...
      return reserve(mapId,requestedSize,fetcher,ReservePreference.MEMORY);
  }
  
  public synchronized MapOutput<K,V> reserve(TaskAttemptID mapId, 
                                             long requestedSize,
                                             int fetcher,
                                             ReservePreference pref
                                             ) throws IOException {
    if (!canShuffleToMemory(requestedSize) || pref == ReservePreference.DISK ) {
    ...
    if (usedMemory > memoryLimit) {
        if ( pref ==  ReservePreference.MEMORY ) {
            if ( LOG.isDebugEnabled() )
                LOG.debug(mapId + ": Stalling shuffle since usedMemory (" + usedMemory + 
                    ") is greater than memoryLimit (" + memoryLimit + ")"); 
           return stallShuffle;
        } else { // should be PREFER_MEMORY. just go DISK
            return new MapOutput<K,V>(mapId, this, requestedSize, jobConf, localDirAllocator, fetcher, true);
        }
    }
    
    // FIXME what if the top memory is just a little bit too high?
    
    // inlining unconditional reserve to handle OOM

    MapOutput<K,V> mo = stallShuffle;
    usedMemory += requestedSize;
    if ( usedMemory < oomWaterMark ) {
        // Allow the in-memory shuffle to progress
        LOG.info(mapId + ": Proceeding with shuffle since usedMemory (" +
            usedMemory + 
            ") is lesser than memoryLimit (" + memoryLimit + ")"); 

        try {
            mo = new MapOutput<K,V>(mapId, this, (int)requestedSize, true);
        } catch ( OutOfMemoryError oom ) {
            mo = stallShuffle; // being paranoid.
            oomWaterMark = Math.min(oomWaterMark, usedMemory);
        }
    }
    
    if ( mo == stallShuffle ) {
        // rollback used memory
        usedMemory -= requestedSize;
        if ( LOG.isDebugEnabled() ) {
            LOG.debug(mapId + ": Stalling shuffle since it reach beyond OOM water mark ("+oomWaterMark+")");
        }
    }
  ...
  private RawKeyValueIterator finalMerge(JobConf job, FileSystem fs,
    ...
    if (0 != onDiskBytes) {
      ...
      RawKeyValueIterator diskMerge = Merger.merge(
          ...
          reporter, false, spilledRecordsCounter, null, thisPhase, finalSegments.isEmpty() ? listener : null);
      diskSegments.clear();
