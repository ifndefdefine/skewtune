...
public class TokenCache {
  ...
  public static void addSecretKey(Text alias, byte[] key) {
    addSecretKey(getTokenStorage(), alias, key);
  }
  public static void addSecretKey(TokenStorage storage,Text alias, byte[] key) {
      storage.addSecretKey(alias, key);
  }
  ...
  public static void addDelegationToken(
    ...
    addDelegationToken(getTokenStorage(),namenode,t);
  }

  public static void addDelegationToken(TokenStorage storage,
          String namenode, Token<? extends TokenIdentifier> t) {
        storage.addToken(new Text(namenode), t);
  }
  ...
  throws IOException {
    obtainTokensForNamenodes(getTokenStorage(), ps, conf);
  }

  public static void obtainTokensForNamenodes(TokenStorage storage,Path [] ps, Configuration conf) 
  throws IOException {
    if (!UserGroupInformation.isSecurityEnabled()) {
      return;
    }
    obtainTokensForNamenodesInternal(storage, ps, conf);
  }
  
  static void obtainTokensForNamenodesInternal(TokenStorage storage,Path [] ps, Configuration conf)
  throws IOException {
    ...
    for(Path p: ps) {
      ...
      if(fs instanceof DistributedFileSystem) {
        ...
        Token<DelegationTokenIdentifier> token = 
          TokenCache.getDelegationToken(storage, fs_addr); 
        if(token != null) {
        ...
        TokenCache.addDelegationToken(storage, fs_addr, token);
  ...
  getDelegationToken(String namenode) {
    return getDelegationToken(getTokenStorage(),namenode);
  }

  @SuppressWarnings("unchecked")
  @InterfaceAudience.Private
  public static Token<DelegationTokenIdentifier> 
  getDelegationToken(TokenStorage storage, String namenode) {
    return (Token<DelegationTokenIdentifier>)storage.
